...
export class DataElementUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formData = {
      ...
      elementCode: this.data.item.elementCode,
      elementName: this.data.item.elementName,
      dataType: this.data.item.dataType,
      dataLength: this.data.item.dataLength,
      dataPrecision: this.data.item.dataPrecision,
      dataScale: this.data.item.dataScale,
      nullable: this.data.item.nullable,
      dataDefault: this.data.item.dataDefault,
      lowValue: this.data.item.lowValue,
      highValue: this.data.item.highValue,
  ...
  submitForm({ valid }) {
    ...
    if (valid) {
      this.metaElementService.update(d).subscribe((d) => {
        if (d.success) {
          this.data.onClose();
          this.data.refresh();
        }
