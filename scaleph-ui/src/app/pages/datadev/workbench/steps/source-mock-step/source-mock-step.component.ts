...
@Component({
  selector: 'app-source-mock-step',
  templateUrl: './source-mock-step.component.html',
  styleUrls: ['../../workbench.component.scss'],
})
export class SourceMockStepComponent implements OnInit {
  @Input() data;
  ...
  defaultMockDataSize: number = 300;
  ...
  formGroup: FormGroup = this.fb.group({
    stepTitle: [null],
    mock_data_size: [this.defaultMockDataSize],
  });
  ...
  ngOnInit(): void {
    ...
    this.jobService.listStepAttr(this.jobId, this.stepCode).subscribe((d) => {
      ...
      if (stepAttrMap.get(STEP_ATTR_TYPE.mockDataSize)) {
        this.formGroup.patchValue({ mock_data_size: stepAttrMap.get(STEP_ATTR_TYPE.mockDataSize) });
      } else {
        this.formGroup.patchValue({ mock_data_size: this.defaultMockDataSize });
      }
  ...
  submitForm() {
    ...
    if (this.formGroup.valid) {
      ...
      stepAttrMap.set(STEP_ATTR_TYPE.mockDataSize, this.formGroup.get(STEP_ATTR_TYPE.mockDataSize).value);
