...
@Component({
  selector: 'app-druid-datasource',
  templateUrl: './druid-datasource.component.html',
  styleUrls: ['../datasource.component.scss'],
})
export class DruidDatasourceComponent implements OnInit {
  parent: HTMLElement;
  @Input() data: any;
  ...
  ngOnInit(): void {
    ...
    if (this.data.item.item) {
      this.formData = {
        ...
        jdbcUrl: this.data.item.item.props.jdbc_url,
  ...
  submitForm({ valid }) {
    let ds: MetaDataSource = {
      ...
      props: {
        jdbc_url: this.formData.jdbcUrl,
      },
