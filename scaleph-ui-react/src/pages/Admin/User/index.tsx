...
import { deleteUserBatch, deleteUserRow, listUserByPage } from "@/services/admin/user.service";
import { DeleteOutlined, EditOutlined, PlusOutlined, UserSwitchOutlined } from "@ant-design/icons";
import { ActionType, ProColumns, ProFormInstance, ProTable } from "@ant-design/pro-components";
import { Button, Card, Col, List, message, Modal, Row, Space, Tabs, Tooltip, Typography } from "antd"
...
const User: React.FC = () => {
    ...
    const [userFormData, setUserFormData] = useState<{ visiable: boolean, data: SecUser }>({ visiable: false, data: {} });
    const [roleFormData, setRoleFormData] = useState<{ visiable: boolean, data: SecRole }>({ visiable: false, data: {} });
    const [deptFormData, setDeptFormData] = useState<{ visiable: boolean, data: SecDept }>({ visiable: false, data: {} });
    ...
    const tabBarButtonOperations = (type: string) => {
        const intl = useIntl();
        if (type == 'role') {
            return <Tooltip title={intl.formatMessage({ id: 'app.common.operate.new.label' }) + intl.formatMessage({ id: 'pages.admin.user.role' })}>
                <Button
                    shape="default"
                    type="link"
                    icon={<PlusOutlined />}
            ...
            return <Tooltip title={intl.formatMessage({ id: 'app.common.operate.new.label' }) + intl.formatMessage({ id: 'pages.admin.user.dept' })}>
                <Button
                    shape="default"
                    type="link"
                    icon={<PlusOutlined />}
                            ...
                            <List
                                ...
                                renderItem={item => (
                                    ...
                                    >
                                        ...
                                        {item.showOpIcon && <Space size={2} >
                                            <Button
                                                shape="default"
                                                type="text"
                                                size="small"
                                                icon={<EditOutlined />}
                                                onClick={() => { setRoleFormData({ visiable: true, data: item }) }}>
                                            </Button>
                                            <Button
                                                shape="default"
                                                type="text"
