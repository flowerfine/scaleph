...
import { grantDeptToUsers } from "@/services/admin/dept.service";
import { SecDept, SecUser } from "@/services/admin/typings";
import { listByUserNameAndDept, listUserByPage } from "@/services/admin/user.service";
...
const DeptGrant: React.FC<ModalFormProps<SecDept>> = ({
    ...
    useEffect(() => {
        ...
        listByUserNameAndDept('', data.id + '', "1").then(resp => {
    ...
    return (
        <Modal
            ...
            onCancel={onCancel}
            onOk={() => {
                grantDeptToUsers(data.id + '', targetKeys).then(d => {
                    if (d.success) {
