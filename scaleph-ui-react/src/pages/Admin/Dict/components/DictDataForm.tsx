...
import { addDictData, updateDictData } from "@/services/admin/dictData.service";
import { useEffect, useState } from "react";
import { listAllDictType } from "@/services/admin/dictType.service";
...
const DictDataForm: React.FC<ModalFormProps<SysDictData>> = ({
    ...
    return (
        <Modal
            ...
            title={
                data.id ?
                    intl.formatMessage({ id: 'app.common.operate.edit.label' }) + intl.formatMessage({ id: 'pages.admin.dict.dictData' }) :
                    intl.formatMessage({ id: 'app.common.operate.new.label' }) + intl.formatMessage({ id: 'pages.admin.dict.dictData' })
            }
            width={580}
            destroyOnClose={true}
            onCancel={onCancel}
            onOk={() => {
                form.validateFields().then((values) => {
                    ...
                    data.id ?
                        updateDictData({ ...dictData }).then(d => {
                            if (d.success) {
                                message.success(intl.formatMessage({ id: 'app.common.operate.edit.success' }));
                                onVisibleChange(false);
                            }
                        }) :
                        addDictData({ ...dictData }).then(d => {
                            if (d.success) {
