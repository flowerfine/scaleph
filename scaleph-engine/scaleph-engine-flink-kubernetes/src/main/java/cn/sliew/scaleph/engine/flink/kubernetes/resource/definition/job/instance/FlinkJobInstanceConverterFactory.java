...
public class FlinkJobInstanceConverterFactory {
    ...
    private Optional<FlinkJobInstanceConverter> findConverter(WsFlinkKubernetesJobDTO jobDTO) {
        return registry.values().stream()
                .filter(converter -> converter.getDeploymentKind() == jobDTO.getDeploymentKind())
                .filter(converter -> converter.getFlinkJobType() == jobDTO.getType())
                .filter(converter -> converter.getFlinkVersion() == getFlinkVersion(jobDTO))
                .findAny();
    }
