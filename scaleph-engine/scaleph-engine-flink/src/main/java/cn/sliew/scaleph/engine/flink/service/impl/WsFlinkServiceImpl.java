...
public class WsFlinkServiceImpl implements WsFlinkService {
    ...
    private ClusterClient doSubmitSeatunnel(WsFlinkJobDTO wsFlinkJobDTO, Path workspace) throws Exception {
        WsFlinkClusterConfigDTO wsFlinkClusterConfigDTO = wsFlinkJobDTO.getWsFlinkClusterConfig();
        SeaTunnelReleaseDTO seaTunnelRelease = seaTunnelReleaseService.selectByVersion(SeaTunnelVersion.V_2_3_1);
    ...
    public void updateJobInstanceState(WsFlinkJobDTO wsFlinkJobDTO) {
        ...
        if (Objects.isNull(currentJobState) ||
                ...
                currentJobState == FlinkJobState.CREATED ||
                currentJobState == FlinkJobState.RUNNING ||
                currentJobState == FlinkJobState.RESTARTING ||
                currentJobState == FlinkJobState.SUBMITING ||
            ...
            if (webInterfaceUrl != null) {
                ...
                if (flinkJobInstance.getJobState() != currentJobState) {
                    wsFlinkJobInstanceService.update(flinkJobInstance);
                }
            }
        }
    }
