...
public enum Validators {
    ...
    public static final Validator URI_LIST_VALIDATOR = new Validator() {
        ...
        public ValidationResult validate(String subject, String input) {
            ...
            Optional<ValidationResult> invalidUri = Arrays.stream(input.split(","))
                    ...
                    .map(String::trim)
                    .map((uri) -> URI_VALIDATOR.validate(subject, uri)).filter((uri) -> !uri.isValid()).findFirst();
    ...
    public static class FileExistsValidator implements Validator {
        ...
        public ValidationResult validate(final String subject, final String value) {
            ...
            if (!Files.exists(path)) {
                return new ValidationResult.Builder().subject(subject).input(value).valid(false).explanation("File " + value + " does not exist").build();
            }
            if (allowFileOnly && Files.isDirectory(path)) {
                return new ValidationResult.Builder().subject(subject).input(value).valid(false).explanation(value + " is not a file").build();
            }
    ...
    public static Validator createRegexMatchingValidator(final Pattern pattern) {
        return createRegexMatchingValidator(pattern, "Value does not match regular expression: " + pattern.pattern());
    }
    ...
    public static final Validator REGULAR_EXPRESSION_VALIDATOR = createRegexValidator(0, Integer.MAX_VALUE);
