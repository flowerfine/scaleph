...
public interface OamComponentDefinitionConvert extends BaseConvert<OamComponentDefinition, ComponentDefinition> {
    OamComponentDefinitionConvert INSTANCE = Mappers.getMapper(OamComponentDefinitionConvert.class);

    @Override
    default OamComponentDefinition toDo(ComponentDefinition dto) {
        OamComponentDefinition entity = new OamComponentDefinition();
        ...
        ComponentDefinition.Spec spec = dto.getSpec();
        if (spec.getSchematic() != null) {
            entity.setSchematic(JacksonUtil.toJsonString(spec.getSchematic()));
        }
        if (spec.getWorkload() != null) {
            entity.setWorkloadType(JacksonUtil.toJsonString(spec.getWorkload()));
        }
        return entity;
    }

    @Override
    default ComponentDefinition toDto(OamComponentDefinition entity) {
        ComponentDefinition dto = new ComponentDefinition();
        dto.setApiVersion(OamConstants.OAM_API_VERSION);
        dto.setKind(OamConstants.OAM_KIND_COMPONENT_DEFINITION);
        ...
        ComponentDefinition.Spec spec = new ComponentDefinition.Spec();
        ...
        if (StringUtils.hasText(entity.getWorkloadType())) {
            spec.setWorkload(JacksonUtil.parseJsonString(entity.getWorkloadType(), ComponentDefinition.WorkloadTypeDescriptor.class));
        }
        // todo 转换 properties
        dto.setSpec(spec);
        return dto;
